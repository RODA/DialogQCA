...
usage() {
    cat <<USAGE
Usage: ./internal/hetzner/deploy-web.sh [options]
...
variables documented in internal/hetzner/web.env.
USAGE
}
...
ExecStart=/usr/bin/node scripts/web-product-dev-server.js --host $DIALOGFORGE_WEB_HOST --port $DIALOGFORGE_WEB_PORT --product-path $DIALOGFORGE_DEPLOY_PRODUCT_DEST
Restart=always
RestartSec=5
