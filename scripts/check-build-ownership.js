...
const assertAutoUpdatePolicy = function(packageJson) {
    ...
    if (autoUpdate.releaseRepository !== "dusadrian/binaries"
        || autoUpdate.releaseTag !== "dqms") {
        fail("package.json product.autoUpdate must default to the DialogQCA macOS Silicon release.");
    }
    if (webRPackageLibrary.releaseTag !== "dqweb") {
        fail("package.json product.webRPackageLibrary.releaseTag must point to the DialogQCA WebR VFS release.");
    }
};
...
const assertWorkflows = function() {
    ...
    if (!linuxText.includes("default: dqli")
        || !windowsText.includes("default: dqwi")
        || !macosText.includes("default: dqmi")
        || !releaseText.includes("default: dqwi")
        || !buildText.includes("default: dqli")
        || !buildText.includes("default: dqwi")
        || !buildText.includes("default: dqmi")) {
        fail("GitHub release workflows must default to platform-specific DialogQCA release tags.");
    }
