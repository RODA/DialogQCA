...
const baseUrl = String(process.argv[2] || process.env.DIALOGQCA_WEB_URL || "http://127.0.0.1:5173")
    .replace(/\/+$/g, "");
...
const verifyComposition = function(result) {
    ...
    if (composition.product?.id !== "DialogQCA") {
        throw new Error("/api/composition did not return the DialogQCA product composition");
    }
...
const main = async function() {
    console.log(`Verifying DialogQCA Web deployment at ${baseUrl}`);
